...
public class LocationAdapter implements JsonDeserializer<Location>, JsonSerializer<Location> {
	...
	public Location deserialize(JsonElement json, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
		...
		if (!world.isJsonPrimitive() || !((JsonPrimitive) world).isString()) throw new JsonParseException("World is not a string");

		if (!x.isJsonPrimitive() || !((JsonPrimitive) x).isNumber()) throw new JsonParseException("X is not a number");

		if (!y.isJsonPrimitive() || !((JsonPrimitive) y).isNumber()) throw new JsonParseException("Y is not a number");

		if (!z.isJsonPrimitive() || !((JsonPrimitive) z).isNumber()) throw new JsonParseException("Z is not a number");

		if (!yaw.isJsonPrimitive() || !((JsonPrimitive) yaw).isNumber()) throw new JsonParseException("Yaw is not a number");

		if (!pitch.isJsonPrimitive() || !((JsonPrimitive) pitch).isNumber()) throw new JsonParseException("Pitch is not a number");

		World worldInstance = Bukkit.getWorld(world.getAsString());
		if (worldInstance == null) throw new IllegalArgumentException("Unknown/not loaded world");
